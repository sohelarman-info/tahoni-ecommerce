                ...
                <div class="col-lg-6">
                    <div class="product-single-img">
                        <div class="product-active owl-carousel">
                            ...
                            <div class="item">
                                <img src="{{ asset('gallery/'.$img->created_at->format('Y/m/').$img->product_id.'/'. $img->images) }}" alt="{{ $product->title }}">
                            </div>
                            @endforeach
                        </div>
                        <div class="product-thumbnil-active  owl-carousel">
                            ...
                            <div class="item">
                                <img src="{{ asset('gallery/'.$gal->created_at->format('Y/m/').$gal->product_id.'/'. $gal->images) }}" alt="{{ $product->title }}">
                            </div>
                            @endforeach --}}
                        </div>
                    </div>
                ...
                <div class="col-lg-6">
                    <form action="{{ route('AddToCart') }}" method="POST">
                        ...
                        <div class="product-single-content">
                            ...
                            <div class="rating-wrap fix">
                                ...
                                <ul class="rating pull-right">
                                    <div class="star-ratings-sprite">
                                        <span style="width:{{ $reviews->sum('rating')/$reviews->count()*20 }}%" class="star-ratings-sprite-rating"></span>
                                    </div>
                                    ...
                                    <li><i class="fa fa-star"></i></li>
                                    <li><i class="fa fa-star"></i></li>  --}}
                                    <li>({{ $reviews->count() }} Customar Review ({{ $reviews->sum('rating')/$reviews->count()*20 }}) )</li>
                                </ul>
                            </div>
                            <style>
                                .btncolor{
                            ...
                            <p>{{ Str::limit($product->summary, 300) }}</p>
                            ...
                            <ul class="cetagory">
                                <li>Categories:</li>
                                <li>
                                    <a href="#">{{ $product->category->category_name ?? 'N/A' }}, {{ $product->subcategory->subcategory_name ?? 'N/A' }}</a>
                                </li>
                                @php
                                    $attribute = App\attribute::where('product_id', $product->id)->get();
                                @endphp
                                <br>
                                <li>Color:</li>
                                    @foreach ($groupBy as $color)
                                    <input class="color_id" type="radio" data-product="{{ $color[0]->product_id }}" name="color_id" value="{{ $color[0]->color_id }}"> {{ $color[0]->color->color_name }}
                                    @endforeach
                                </li><br>
                                <li>Size:</li>
                                <li class="sizeadd">
                                    @foreach ($product->get_attribute as $size)
                                    <input type="radio" class="size_id" name="size_id" value=""> {{ $size->size->size_name }}
                                    @endforeach
                                </li>
                            </ul>
                            ...
                            <ul class="socil-icon">
                                ...
                                <li><a href="https://twitter.com/intent/tweet?text={{ $product->title }}&url={{ route('SingleProduct',  $product->slug)}}"><i class="fa fa-twitter"></i></a></li>
                            ...
                            <div class="faq-wrap" id="accordion">
                                ...
                                <div class="card">
                                    <div class="card-header" id="headingTwo">
                                        <h5><button class="collapsed" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">How To</button></h5>
                                    </div>
                                    ...
                                    @foreach ($reviews as $review)
                                        <li class="review-items">
                                            ...
                                            <div class="review-content">
                                                ...
                                                <span>{{ $review->created_at->format('d M, Y') }} at {{ $review->created_at->format('h:s a') }}</span>
                                                <p>{{ $review->massage }}</p>
                                                <ul class="rating">
                                                    @if ($review->rating == 1)
                                                    <li><i class="fa fa-star"></i></li>
